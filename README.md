...
# Alternatif: Uzak sunucu modelini kullan
RAG_ENV=remote python chat.py
```
...
2. **Model Değerlendirmesi:**
   - `minutes_jina_v3` vs `minutes_jina_v4` — aynı sorguda iki modelin sonuçlarını yan yana karşılaştırın.
...
Çıktı:
```
Koleksiyon        Model                          Context   Dim   Late Chunking
─────────────────────────────────────────────────────────────────────────────
press_nomic      nomic-embed-text-v2-moe           512    768       ✗
minutes_nomic    nomic-embed-text-v2-moe           512    768       ✗
minutes_jina_v3  jinaai/jina-embeddings-v3        8192   1024       ✓
minutes_jina_v4  jinaai/jina-embeddings-v4       32768   1024       ✓
```
...
| `collection` | Hedef koleksiyon | `minutes_jina_v4` |
...
collections:
  minutes_jina_v4:                    # 2. Koleksiyonu tanımla
...
Kaydettikten sonra `get_spec("minutes_jina_v4")` ile kullanabilirsiniz.
...
spec = get_spec("minutes_jina_v4")
pipe = IngestionPipeline(spec=spec)

doc = DocumentInput(
    document_id="tbmm-20-1-1-19960108",
    document_type="tutanak",
    collection_name="minutes_jina_v4",
...
| `RAG_DEFAULT_COLLECTION` | `tutanaklar_nomic_v2` | `RAGService` varsayılan collection'ı |
...
collections:
  tutanaklar_nomic_v2:
    collection_name: tbmm_tutanaklar_nomic_v2
    chroma_path: data_lake/parliament_digital_born_minutes_vectors
    sqlite_path: data_lake/parliament_digital_born_minutes.db
...
```yaml
defaults:
  gazete: press_jina_v3
  tutanak: tutanaklar_nomic_v2
  onerge: onerge_jina_v3
```
...
[PIPELINE] │  intent: temporal | resources: minutes_nomic
[PIPELINE] ├─ PHASE 2: Retrieval
[PIPELINE] │  minutes_nomic: 5 results | latency: 0.8s
...
#   collections: [minutes_jina_v3, minutes_nomic]
...
python scripts/generate_golden.py --collection minutes_jina_v3 --n 3 --dry-run
```
